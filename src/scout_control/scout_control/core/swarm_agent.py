    ...
    def __init__(self) -> None:
        ...
        if not self._runtime_backend_active:
            self._offboard_pub = self.create_publisher(
                OffboardControlMode,
                topics.px4_in_offboard_control_mode, QOS_PX4_PUB)
            self._traj_pub = self.create_publisher(
                TrajectorySetpoint,
                topics.px4_in_trajectory_setpoint, QOS_PX4_PUB)
            self._cmd_pub = self.create_publisher(
                VehicleCommand,
                topics.px4_in_vehicle_command, QOS_PX4_PUB)
