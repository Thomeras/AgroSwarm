"""
manual_controller.py — Legacy dual-drone manual PX4 flight controller

Deprecated for production autonomy setup. This node is a flight owner: it
registers PX4 offboard/setpoint/vehicle-command publishers and continuously
publishes setpoints until mission handoff. Use field_setup_tool for production
pad/corner/mission-confirm setup because it never publishes PX4 setpoints.
...
Usage:
  ...
  ros2 run scout_control legacy_manual_controller --ros-args -p altitude:=5.0
"""
...
from scout_control.utils.paths import PERIMETERS_DIR
...
class ManualController(Node):
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            f"ManualController ready [LEGACY DEBUG FLIGHT OWNER] | "
            f"altitude={self._altitude} m | 2 drones | "
            f"ui={'on' if self._ui_enabled else 'off'}"
        )
