...
class LocalGridSnapshot:
    ...
    state: str = "TRACKING"
    stamp_s: float = 0.0
    valid_for_planning: bool = True
    validity_reason: str = "tracking"
    ...
    def map_ready(self) -> bool:
        return bool(self.valid_for_planning) and str(self.state).upper() == "TRACKING"
...
class LocalPlanner:
    ...
    def plan(
        ...
        if not grid.map_ready():
            state = str(grid.state or "UNKNOWN")
            reason = str(grid.validity_reason or f"map_state_{state.lower()}")
            planner_state = (
                LocalPlannerState.NO_MAP
                if state.upper() == "EMPTY"
                else LocalPlannerState.DEGRADED
            )
            return PlanResult(
                status=PlannerResultStatus.NO_PATH,
                planner_state=planner_state,
                reason=f"planner_map_not_ready:{state}",
