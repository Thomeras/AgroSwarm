...
class GcsBridge(Node):
    ...
    def __init__(self) -> None:
        ...
        self._mode_pub = self.create_publisher(
            String, "/swarm/mode", QOS_LATCHED_RELIABLE)
        self._peer_cells_pub = self.create_publisher(
            String, "/swarm/peer_cells", QOS_LATCHED_RELIABLE)
        self._rth_pub = self.create_publisher(
            String, "/swarm/rth_request", QOS_VOLATILE_RELIABLE)
        # Milestone 3
        ...
        self._manual_control_pub = self.create_publisher(
            String, "/swarm/manual_control", QOS_VOLATILE_BE)

        # ── Subscribers (ROS2 → Swarm Center) ────────────────────────────────
        self.create_subscription(
            String, "/swarm/task_status",
