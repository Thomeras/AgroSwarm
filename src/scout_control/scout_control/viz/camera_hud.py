...
from scout_control.utils.paths import PERIMETER_FILE
...
class CameraHud(Node):
    ...
    def __init__(self) -> None:
        ...
        topics = TelemetryHub(drone_id=int(self.get_parameter("drone_id").value)).topics
        self._show_minimap: bool = bool(self.get_parameter("show_minimap").value)
        camera_topic: str = str(self.get_parameter("camera_topic").value).strip() or topics.camera_image
        pos_topic:    str = str(self.get_parameter("pos_topic").value).strip() or topics.vehicle_local_position
