...
echo "=== Isaac Phase 1+2 live checks ==="
echo
...
echo "=== Phase 1: swarm_agent should not publish /fmu/in topics ==="
ros2 node info /swarm_agent_0 2>/dev/null | grep -A30 "Publishers" | grep "fmu/in" \
  || echo "OK: no /fmu/in publishers found on /swarm_agent_0"
echo
...
echo "=== Unit checks: Phase 1+2 helper tests ==="
cd "$WS" || exit 1
...
echo
echo "=== Done. Re-run this after pad assignment, boundary close, and mission start. ==="
